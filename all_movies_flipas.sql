...
FROM (SELECT SUBSTR(actors_list, 0, (SELECT INSTR(actors_list,',') FROM casts)) AS actor_1_name,
SELECT SUBSTR('abc,def,ghi', 1 ,INSTR('abc,def,ghi', ',', 1, 1)-1) FROM casts;

 --substring to obtain actor1 from the string in done by listagg()
...
CREATE VIEW all_movies AS
SELECT color, director_name, num_critic_for_reviews, duration, director_facebook_likes, gross, genre AS genres, movie_title, num_voted_users, cast_total_facebook_likes,
				  facenumber_in_poster, keyword AS plot_keywords, movie_imdb_link, num_user_for_reviews,
				  filming_language, country, content_rating, budget, title_year, imdb_score, aspect_ratio, movie_facebook_likes,
       SUBSTR(LISTAGG(actor, ';') WITHIN GROUP (ORDER BY actor), 1, INSTR(LISTAGG(actor, ';') WITHIN GROUP (ORDER BY actor), ';', 1, 1)-1) AS actor_1_name,
       SUBSTR(LISTAGG(actor, ';') WITHIN GROUP (ORDER BY actor), INSTR(LISTAGG(actor, ';') WITHIN GROUP (ORDER BY actor), ';',1, 1)+1, INSTR(LISTAGG(actor, ';') WITHIN GROUP (ORDER BY actor), ';',1, 2)-1) AS actor_2_name,
       SUBSTR(LISTAGG(actor, ';') WITHIN GROUP (ORDER BY actor),(INSTR(LISTAGG(actor, ';') WITHIN GROUP (ORDER BY actor), ';',1, 2)+1)) AS actor_3_name
FROM casts
JOIN players ON (actor = PLAYERS.actor_name)
JOIN movies ON (title = MOVIES.movie_title)
JOIN genres_movies ON (movie_title = GENRES_MOVIES.title)
